...
export function ReportsView() {
  ...
  const filteredReports = useMemo(() => {
    ...
    return reports.filter((report) =>
      [report.participantName, report.participantEmail, report.packageTitle, String(report.total_score)].join(' ').toLowerCase().includes(keyword),
    );
  }, [reports, search]);

  const totalAttempts = reports.length;
  const highestScore = reports.reduce((max, item) => Math.max(max, item.total_score), 0);
  const averageScore = totalAttempts === 0 ? 0 : Math.round(reports.reduce((sum, item) => sum + item.total_score, 0) / totalAttempts);
  const averageListening = totalAttempts === 0 ? 0 : Math.round(reports.reduce((sum, item) => sum + item.listening_score, 0) / totalAttempts);
  const averageStructure = totalAttempts === 0 ? 0 : Math.round(reports.reduce((sum, item) => sum + item.structure_score, 0) / totalAttempts);
  const averageReading = totalAttempts === 0 ? 0 : Math.round(reports.reduce((sum, item) => sum + item.reading_score, 0) / totalAttempts);

  const loadReports = async () => {
    setLoading(true);

    try {
      const { data: certificateRows, error: certificateError } = await supabase.from('certificates').select('*').order('generated_at', { ascending: false });
      if (certificateError) throw certificateError;

      const certificates = (certificateRows || []) as Certificate[];
      ...
      const [profileResult, packageResult] = await Promise.all([
        ...
        packageIds.length ? supabase.from('test_packages').select('id, title').in('id', packageIds) : Promise.resolve({ data: [], error: null }),
      ]);
      ...
      const profileMap = new Map<string, ProfileLite>(profileRows.map((row: ProfileLite) => [row.id, row]));
      const packageMap = new Map<string, PackageLite>(packageRows.map((row: PackageLite) => [row.id, row]));

      const nextReports: ReportRow[] = certificates.map((certificate: Certificate) => ({
        ...certificate,
  ...
  const exportCsv = () => {
    const header = ['Participant Name', 'Participant Email', 'Package', 'Listening', 'Structure', 'Reading', 'Total', 'Generated At'];
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
          <input type="search" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search participant, package, or score" className="field-input w-full sm:min-w-[260px]" />
          <button onClick={exportCsv} disabled={filteredReports.length === 0} className="secondary-btn w-full disabled:cursor-not-allowed disabled:opacity-50 sm:w-auto">
      ...
      <div className="admin-surface overflow-hidden">
        <div className="border-b border-[rgba(119,123,179,0.14)] px-4 py-4 sm:px-6">
        ...
        {filteredReports.length === 0 ? (
          ...
          <>
            ...
            <div className="space-y-3 p-4 md:hidden">
              {filteredReports.map((report) => (
                <article key={report.id} className="admin-soft-surface p-4">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <div className="text-base font-extrabold text-[color:var(--ink-strong)]">{report.participantName}</div>
                      <div className="mt-1 text-xs text-[color:var(--ink-soft)]">{report.participantEmail}</div>
                    </div>
                    <span className="inline-flex items-center gap-2 rounded-full bg-[color:var(--blue-soft)] px-3 py-1 text-xs font-semibold text-[color:var(--blue-deep)]">
                      <Trophy className="h-3.5 w-3.5" />
                      {report.total_score}
                    </span>
                  </div>
                  <div className="mt-4 space-y-2 text-sm text-[color:var(--ink-main)]">
                    <div>Package: {report.packageTitle}</div>
                    <div>Listening: {report.listening_score}</div>
                    <div>Structure: {report.structure_score}</div>
                    <div>Reading: {report.reading_score}</div>
                    <div className="text-xs text-[color:var(--ink-soft)]">Generated: {new Date(report.generated_at).toLocaleString()}</div>
                  </div>
                </article>
              ))}
            </div>
...
function MetricCard({ label, value }: { label: string; value: number }) {
  return (
    <div className="admin-soft-surface p-5">
      ...
      <div className="mt-3 text-3xl font-extrabold text-[color:var(--ink-strong)] sm:text-4xl">{value}</div>
    </div>
  );
}
