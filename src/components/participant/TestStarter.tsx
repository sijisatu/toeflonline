...
export function TestStarter({ packageId }: TestStarterProps) {
  ...
  const fetchPackageInfo = async () => {
    try {
      const { data: packageData, error: packageError } = await supabase.from('test_packages').select('*').eq('id', packageId).single();
      if (packageError) throw packageError;
      setTestPackage(packageData);

      const { data: sectionsData, error: sectionsError } = await supabase.from('test_sections').select('*').eq('package_id', packageId).order('section_order');
      ...
      <div className="shell-wrap space-y-5 sm:space-y-6">
        ...
        <section className="grid gap-5 xl:grid-cols-[1.08fr_0.92fr] xl:gap-6">
          <div className="glass-card p-5 sm:p-8">
            ...
            <h1 className="mt-4 text-3xl font-extrabold sm:text-4xl">{testPackage?.title}</h1>
            {testPackage?.description && (
              <p className="mt-4 max-w-3xl text-sm leading-8 text-[color:var(--ink-soft)] md:text-base">{testPackage.description}</p>
            )}
            ...
            <div className="mt-6 grid gap-4 lg:mt-8 lg:grid-cols-[1fr_1fr]">
              <div className="soft-card p-5">
                <div className="text-sm font-bold uppercase tracking-[0.24em] text-[color:var(--ink-soft)]">Section Order</div>
                <div className="mt-4 space-y-3">
                  {sections.map((section, index) => (
                    <div key={section.id} className="flex flex-col gap-2 rounded-[20px] bg-white px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
                      <div>
              ...
              <div className="rounded-[24px] bg-gradient-to-br from-[#f8edcf] to-[#fff7e6] p-5 sm:rounded-[28px]">
                <div className="flex items-start gap-3">
                  <AlertCircle className="mt-0.5 h-5 w-5 shrink-0 text-[#a96700]" />
                  <div>
          ...
          <div className="glass-card-strong p-5 sm:p-8">
            ...
            <h2 className="mt-4 text-2xl font-extrabold sm:text-3xl">Verify your device access.</h2>
            <p className="mt-3 text-sm leading-7 text-[color:var(--ink-soft)]">
              Before entering the test, confirm that camera and microphone permissions are available.
            </p>

            <div className="mt-8 space-y-4">
              <PermissionCard active={cameraPermission} icon={<Camera className="h-5 w-5" />} title="Camera Access" description="Used for presence monitoring and periodic capture during the session." />
              <PermissionCard active={micPermission} icon={<Mic className="h-5 w-5" />} title="Microphone Access" description="Used to confirm the session environment remains compliant." />
            </div>

            <div className="mt-8 space-y-3">
              {!cameraPermission || !micPermission ? (
                <button onClick={requestPermissions} className="primary-btn w-full">Grant Permissions</button>
              ) : (
                <button onClick={startTest} className="primary-btn w-full">Start Test</button>
              )}
              <button onClick={() => (window.location.href = '/')} className="secondary-btn w-full">Return to Dashboard</button>
            </div>
          </div>
        </section>
      </div>
...
function PermissionCard({ active, icon, title, description }: { active: boolean; icon: ReactNode; title: string; description: string }) {
  return (
    <div className={`rounded-[24px] p-5 sm:rounded-[26px] ${active ? 'bg-[#effff6]' : 'bg-[#f8f8ff]'}`}>
      <div className="flex items-start gap-4">
        <div className={`flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl ${active ? 'bg-[#d5ffe9] text-[#13884f]' : 'bg-white text-[color:var(--blue-deep)]'}`}>
          {icon}
        </div>
        <div className="min-w-0 flex-1">
          <div className="flex items-center justify-between gap-3">
            <h3 className="text-base font-extrabold">{title}</h3>
            {active && <CheckCircle2 className="h-5 w-5 shrink-0 text-[#18a561]" />}
          </div>
